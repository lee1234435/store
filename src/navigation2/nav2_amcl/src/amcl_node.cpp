...
void AmclNode::apriltagPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
    std::lock_guard<std::recursive_mutex> cfl(mutex_);
    RCLCPP_INFO(
        get_logger(), "Received AprilTag Pose for Initial Pose Handling: x = %f, y = %f, z = %f, w = %f",
        msg->pose.orientation.x,
        msg->pose.orientation.y,
        msg->pose.orientation.z,
        msg->pose.orientation.w);
    
    // Create a PoseWithCovarianceStamped message to be passed to AMCL
    geometry_msgs::msg::PoseWithCovarianceStamped pose_with_cov;
    pose_with_cov.header = msg->header;
    pose_with_cov.pose.pose = msg->pose;
    
    // Set a default covariance
    for (int i = 0; i < 36; ++i) {
        pose_with_cov.pose.covariance[i] = 0.0;
    }
    pose_with_cov.pose.covariance[0] = 0.5 * 0.5;   // X covariance
    pose_with_cov.pose.covariance[7] = 0.5 * 0.5;   // Y covariance
    pose_with_cov.pose.covariance[35] = (M_PI / 12.0) * (M_PI / 12.0);  // Yaw covariance
    
    // Forward the transformed pose to the original handleInitialPose logic
    handleInitialPose(pose_with_cov);
}


// void AmclNode::correctedPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
// {
//     std::lock_guard<std::recursive_mutex> cfl(mutex_);
//     RCLCPP_INFO(
//         get_logger(), "Received Corrected Pose: x = %f, y = %f, z = %f, w = %f",
//         msg->pose.orientation.x,
//         msg->pose.orientation.y,
//         msg->pose.orientation.z,
//         msg->pose.orientation.w);
...
//     // Set a default covariance (or you can adjust this depending on your confidence in the data)
//     for (int i = 0; i < 36; ++i) {
//         pose_with_cov.pose.covariance[i] = 0.0;
//     }
//     pose_with_cov.pose.covariance[0] = 0.5 * 0.5;   // X covariance
...
AmclNode::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  apriltag_pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
        "apriltag_pose", rclcpp::SystemDefaultsQoS(),
        std::bind(&AmclNode::apriltagPoseCallback, this, std::placeholders::_1));

  
  // corrected_pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
  //       "corrected_pose", rclcpp::SystemDefaultsQoS(),
  //       std::bind(&AmclNode::correctedPoseCallback, this, std::placeholders::_1));
